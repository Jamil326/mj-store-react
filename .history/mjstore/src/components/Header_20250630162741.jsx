...
const Header = () => {
  ...
  const navLinks = [
    ...
    { path: "dashboard", label: "Acco", icon: <MdDashboard />, condition: logged },
      ...
      <Container fluid>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="ms-auto w-100">
            <Row className="d-lg-none w-100 text-center">
              {navLinks
                .filter((link) => link.condition === undefined || link.condition)
                .map((link, idx) => (
                  <Col xs={6} key={idx} className="my-2">
            ...
            <div className="d-none d-lg-flex">
              {navLinks
                .filter((link) => link.condition === undefined || link.condition)
                .map((link, idx) => (
                  <Nav.Link key={idx} as={NavLink} to={link.path} className="fw-bold text-dark d-flex align-items-center">
