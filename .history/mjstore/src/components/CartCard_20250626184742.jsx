  ...
  const updateQuantity = async (type) => {
    ...
    try {
      ...
      if (res.ok && result.status === "success") {
        // Find the updated item from the items array
        const updatedProduct = result.data.items.find((item) => item.product._id === _id);
        if (updatedProduct) {
          setQuantity(updatedProduct.quantity); // Update quantity
          onCartUpdate(result.data.items); // Notify parent with the updated cart data
          toast.success(`Quantity ${type}d successfully.`);
        } else {
          throw new Error("Updated product not found in response.");
        }
      } else {
