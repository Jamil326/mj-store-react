...
const OrderPage = () => {
  ...
  const handleSub = () => {
    return setNoItem((prev)=>Math.max(prev-1,1))
  }
  ...
  const addToCartAndOrder = async () => {
    try {
      ...
      const cartResponse = await fetch(cartUrl, {
        ...
        body: JSON.stringify({ productId: item._id,quantity:noItem }),
      });
        ...
        <Col>
          <ListGroup>
            ...
            <ListGroup.Item>
              ...
              <Button onClick={handleSub} variant='success ms-5 py-1'>-</Button>
              <span className=' mx-2  fs-2  border text-center px-3 pb-2 '>{noItem}</span>
            ...
            <ListGroup.Item>
              <strong>Total:</strong> ₹{(item?.price *noItem || 0) * quantity}
            </ListGroup.Item>
          </ListGroup>
        </Col>
      </Row>
