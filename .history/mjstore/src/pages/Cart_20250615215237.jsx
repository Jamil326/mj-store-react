...
const Cart = () => {
    ...
    const getData =async () => {
        ...
        try {
            ...
            if(res.status===200){
                ...
                setQuantity( result.data.items[0].quantity );
  ...
  const HandleDone = () => {
       dir('/checkout',{ state:{ item:items[0].product, quantity:items[0] }});
  }
      ...
      <div>
          {
              items.length>0?items.map(( item, index)=> (
                  <div key={index}>
                     ...
                     <CartCard product={ {product:item.product,quantity:quantity} }  />
