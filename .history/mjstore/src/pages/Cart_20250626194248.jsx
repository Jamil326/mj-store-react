...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      // Update cart data and calculate total
      const items = result.data.items || [];
      setCartData(items);
      setTotalValue(items.reduce((sum, item) => sum + item.product.price * item.quantity, 0));
    } catch (error) {
  ...
  const handleRemoveFromCart = (productId) => {
    const updatedCart = cartData.filter((item) => item.product !== productId);
    setCartData(updatedCart);
    setTotalValue(updatedCart.reduce((sum, item) => sum + item.product.price * item.quantity, 0));
  };

  // Navigate to the checkout page
  const handleCheckout = () => {
    const itemIds = cartData.map((item) => item.product);
          ...
          <div className="d-flex flex-wrap gap-3">
            {cartData.map((item) => (
              <CartCard
                ...
                onCartUpdate={setCartData} // Directly pass setCartData for updates
              />
            ))}
          </div>
