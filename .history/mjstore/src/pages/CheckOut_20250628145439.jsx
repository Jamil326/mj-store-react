...
const OrderPage = () => {
  ...
  const { name, mobile, city, state, pin, landmark, street } = userAddress || {};
  ...
  const fetchAddressFromBackend = async () => {
    try {
      ...
      const response = await fetch("https://mj-store.onrender.com/api/v1/user/fetchUser", {
        headers: {
      ...
      console.log('address',result.data.shippingAddress);
