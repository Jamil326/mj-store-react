...
const OrderPage = () => {
  ...
  const fetchAddressFromBackend = async () => {
    try {
      ...
      const response = await fetch("https://mj-store.onrender.com/api/v1/user/address", {
        headers: {
      ...
      updateAddressState(result.data);
    } catch (error) {
  ...
  const handleAddressAddedOrUpdated = async (newAddress) => {
    toggleModal();
