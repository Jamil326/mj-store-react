...
import { FaStar, FaHeart, FaSmile, FaCoffee, FaRocket } from "react-icons/fa";
...
const SignUp = () => {
  ...
  const icons = [FaStar, FaHeart, FaSmile, FaCoffee, FaRocket];

  const renderRandomIcons = () => {
    return Array.from({ length: 20 }).map((_, index) => {
      const Icon = icons[Math.floor(Math.random() * icons.length)];
      const randomX = Math.random() * 90; // Random position (horizontal)
      const randomY = Math.random() * 90; // Random position (vertical)
      const randomSize = Math.random() * 40 + 20; // Random size
