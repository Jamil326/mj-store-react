...
const Cart = () => {
  ...
  const handleIncrease = async (productId) => {
    const endpoint = "https://mj-store.onrender.com/api/v1/user/cart/increase";
    await modifyCart(endpoint, productId);
  };

  const handleDecrease = async (productId) => {
    const endpoint = "https://mj-store.onrender.com/api/v1/user/cart/decrease";
    await modifyCart(endpoint, productId);
  };
  ...
  const modifyCart = async (endpoint, productId) => {
    try {
      const token = localStorage.getItem("token");
      if (!token) throw new Error("Invalid token. Please log in.");
      ...
      {cartData.length > 0 ? (
        <>
          <div className="d-flex justify-content-between align-items-center p-3">
            <Button onClick={() => modifyCart("https://mj-store.onrender.com/api/v1/user/cart/clear")} variant="danger">
              Clear Cart
            </Button>
          </div>
