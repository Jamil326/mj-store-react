...
const OrderPage2 = () => {
  ...
  const { city, state, pin, landmark, street } = address || {};

  const toggleAddressForm = () => setVisible((prev) => !prev);
  const redirectToLogin = () => navigate("/login");

  const handleAddressAddedOrUpdated = (newAddress) => {
        ...
        <Col>
          {address ? (
            <Card className="shadow-sm mb-4">
              ...
              <Card.Footer>
                <Button variant="primary" className="w-100" onClick={toggleAddressForm}>
                  Edit Address
                </Button>
              </Card.Footer>
            </Card>
