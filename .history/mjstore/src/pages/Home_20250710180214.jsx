...
const Home = () => {
  ...
  const fetchProducts = useCallback(
    async (reset = false, newPage = 1) => {
      ...
      try {
        ...
        const fetchedProducts = data.data.getProduct.map((p) => ({
          ...p,
          rating: 4
        }));
  ...
  const searchProductsFromAPI = async (query) => {
    try {
      ...
      if (res.ok && data?.data?.getProduct) {
        const searched = data.data.getProduct.map((p) => ({
          ...p,
          rating: 4
        }));
  ...
  return (
    <div className="d-flex flex-column bg-light">
              ...
              >
                <Card
                  className={`p-3 shadow-sm rounded ${
                    isSelected ? "border border-success bg-light" : ""
                  }`}
                  style={{ cursor: "pointer", transition: "all 0.3s ease" }}
                >
                  <div
                    className={`text-center fs-2 ${
                      isSelected ? "text-success" : "text-dark"
                    }`}
                  >
                    {category.icon}
                  </div>
      ...
      <Suspense fallback={<div className="text-center">Loading banner...</div>}>
        <Banner />
      </Suspense>
